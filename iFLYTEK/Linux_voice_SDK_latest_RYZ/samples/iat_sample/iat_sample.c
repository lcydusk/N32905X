...
void run_iat(const char* audio_file, const char* session_begin_params)
{
	...
	while (1) 
	{
		unsigned int len = 10 * FRAME_LEN *3 ; // 每次写入200ms音频(16k，16bit)：1帧音频20ms，10帧=200ms。16k采样率的16位音频，一帧的大小为640Byte
		int ret = 0;
...
int main(int argc, char* argv[])
{
	...
	const char* login_params			=	"appid = 58e4c070, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
    char filename[128] = {0};
	...
	*/
	...
	if(argc < 2){
		strcpy(filename, "wav/iflytek02.wav");
	}
	else{
		strcpy(filename, argv[1]);
	}
		
	run_iat(filename, session_begin_params); //iflytek02音频内容为“中美数控”；如果上传了用户词表，识别结果为：“中美速控”。
exit:
