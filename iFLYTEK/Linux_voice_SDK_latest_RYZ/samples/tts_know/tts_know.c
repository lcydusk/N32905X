...
int main(int argc, char* argv[])
{
	...
	*/
	...
	printf("###########################################################################\n\n");
	/* 文本合成 */
	timestamp(); printf("Start TTS ...\n");
	ret = text_to_speech(text, filename, session_begin_params);
	if (MSP_SUCCESS != ret)
	{
		printf("text_to_speech failed, error code: %d.\n", ret);
	}
	timestamp(); printf("Finish TTS\n");
	
	//play the wav and rm it
	//sprintf(s_cmd_play, "aplay %s && rm -f %s", filename, filename);
	sprintf(s_cmd_play, "./aplay %s", filename);
