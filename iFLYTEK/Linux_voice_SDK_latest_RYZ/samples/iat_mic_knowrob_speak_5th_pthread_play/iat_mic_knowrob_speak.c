...
#define AUDIO_BUF_SIZE          40000
static unsigned char audio_buf[AUDIO_BUF_SIZE] = {0}; 
...
static void play_audio()
{
    do {
        sem_wait(&sem_audio);
        timestamp(); printf("sjdb: start playing().\n");
        
        //void* WavePtr = data4paly; 
		void* WavePtr = &audio_buf[0]; 
        int WaveSize = (audio_len < AUDIO_BUF_SIZE) ? (audio_len/2) : (AUDIO_BUF_SIZE/2) ; 
...
int text_to_speech(const char* src_text/*, const char* des_path, const char* params*/)
{
    ...
    while (1) 
    {
        ...
        if (NULL != data)
        {
            timestamp(); printf("sjdb: New data arrived, audio_len is %d.\n", audio_len);
            sem_wait(&sem_available);           
            memcpy(audio_buf, data, (audio_len < AUDIO_BUF_SIZE) ? audio_len : AUDIO_BUF_SIZE );   //sj new3
            sem_post(&sem_audio);              //sj new3
            continue;
        }
        if (MSP_TTS_FLAG_DATA_END == synth_status){
		    sem_wait(&sem_available);  //sj new3, wait for play finished.
            break;
        }
    ...
    /* 修正wav文件头数据的大小 */
    //wav_hdr.size_8 += wav_hdr.data_size + (sizeof(wav_hdr) - 8);  //sj old2
