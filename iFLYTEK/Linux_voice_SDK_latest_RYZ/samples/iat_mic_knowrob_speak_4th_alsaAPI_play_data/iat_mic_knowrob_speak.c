...
/* sj new2 play_audio*/ 
static void play_audio(void* WavePtr, unsigned int WaveSize)
{
    timestamp(); printf("sjdb: start playing().\n");
    register snd_pcm_sframes_t      count, frames;
    ...
    do
    {
        ...
        if (frames < 0){
            printf("Error playing wave, err %d : %s\n", frames, snd_strerror(frames));
...
int text_to_speech(const char* src_text/*, const char* des_path, const char* params*/)
{
    ...
    while (1) 
    {
        ...
        if (NULL != data)
        {
            //fwrite(data, audio_len, 1, fp); //sj old2 
            timestamp(); printf("sjdb: New data arrived, audio_len is %d.\n", audio_len);
            play_audio(data, audio_len/2);   //!!!!sj 2 BytePerSample
            continue;
        }
